...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # inds = np.argsort(-np.max(overall_scores[:, 1:], axis=-1))[: 1200]
        # pred_rel_inds = pred_rel_inds[inds]
        # rel_scores = rel_scores[inds]
        # overall_scores = overall_scores[inds]

        if (mode == 'predcls' or mode == 'sgcls') and cfg.MODEL.ROI_RELATION_HEAD.REL_NMS:
            pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, 0.6)
            # pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores) ## this version's performance is better
...
def rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, nms_thresh=0.5, l21_thr=0.7):
    ...
    is_overlap = (rel_ious >= nms_thresh) & (sub_labels[:, None] == sub_labels[None, :]) & (
                obj_labels[:, None] == obj_labels[None, :]) & (l21 > l21_thr)
...
### this is better from Leveraging Predicate and Triplet Learning for Scene Graph Generation
def rel_nms_2(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores):
    ious = bbox_overlaps(pred_boxes, pred_boxes)
    sub_ious = ious[pred_rel_inds[:, 0]][:, pred_rel_inds[:, 0]]
    obj_ious = ious[pred_rel_inds[:, 1]][:, pred_rel_inds[:, 1]]
    rel_ious = np.minimum(sub_ious, obj_ious)
    ...
    is_overlap = (rel_ious >= 0.5) & (sub_labels[:, None] == sub_labels[None, :]) & (
            obj_labels[:, None] == obj_labels[None, :])
    is_overlap = is_overlap[:, :, None].repeat(rel_scores.shape[1], axis=2)
    rel_scores_cp = overall_scores.copy()
    rel_scores_cp[:, 0] = 0.
    pred_rels = np.zeros(rel_scores_cp.shape[0], dtype=np.int64)
